...
TextureRectPlayerData Player::texture_rect_player_data;
...
setShapeParametrs()
{
    ...
    Player::shape.setPosition(centre.x - Player::collision_data.lenght / 2,
                              centre.y - Player::collision_data.height / 2);
...
Player::draw(sf::RenderWindow& window)
{
    ...
    sf::Vector2f centre_in_window = DataBase::Centre();

    sf::Vector2f centre_on_map = getCentre();

    sf::Vector2f stop_coordinate_in_window =
        centre_in_window + last_stop.getCoordinate() - centre_on_map;

    shape.setTextureRect(texture_rect_player_data.getTextureRect());
...
Player::getCentre()
{
    return sf::Vector2f(collision_data.coordinate.x + collision_data.lenght / 2,
...
Player::changePosition(sf::Vector2i mouse_position)
{
    ...
    sf::Vector2f player_coordinate_shape = {
        centre.x - collision_data.lenght / 2,
        centre.y + collision_data.height / 2 - collision_data.width};
